...
Le logiciel supporte les modèles temporels suivants :
...
Le logiciel les catégories temporelles suivantes :
...
UHF-SQL est développé en Java (compatible avec Java 17) avec des libraires externes suivantes :
...
Le logiciel prend en entrée un fichier de configuration avec des paramètres
...
* Le mode de mise en oeuvre du schéma historicisé ((virtual | physical | both; pour création des tables ou des vues)
* Si `virtuel`, définir les dépendances fonctionnelles temporelles
* Répertoire de sortie des scripts générés.

=== Sorties

Le logiciel génère un ensemble de scripts pour le schéma historicisé :
...
* La création des fonctions de vérifications des clés référentielles temporelles.
...
* Création des fonctions de vérification des clés référentielles temporelles pour les catégories temporelles transaction (T) et bitemporelle (VT).
* Création des fonctions de vérification des clés référentielles temporelles pour la catégorie de validité (V) pour les modèles BCDM et DDLM.
* Création des vues associées aux anomalies temporelles et des contraintes liées aux anomalies temporelles pour la catégorie temporelle bitemporelle (VT).
* Support de la partition temporelle Vxx lorsque `includeVxx` est à `true`.
* Génération du schéma complet lorsque `useMode` est à `virtual`.
...
. Télécharger l'exécutable (ou le dépôt au complet).
. Dans un terminal exécuter : +
   `java -jar uhf-sql-0.2.0-SNAPSHOT.jar <fichier-de-configuration>`

Pour des exemples de démonstration, consulter le link:demo/lisezmoi.adoc[lisezmoi de démonstration].
...
HORA - Groupe de recherche en modélisation et intégration temporelles de données hétérogènes; +
